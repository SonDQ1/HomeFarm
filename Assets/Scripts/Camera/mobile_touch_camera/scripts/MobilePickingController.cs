...
namespace BitBenderGames {
  ...
  public class MobilePickingController : MonoBehaviour {
    ...
    private float GetPositionXSnapped(float position, float snapOffset)
    {
        if (snapToGrid == true)
        {
            return (Mathf.RoundToInt(position / snapUnitSize) * snapUnitSize) + snapOffset;
    ...
    private float GetPositionYSnapped(float position, float snapOffset)
        {
            if (snapToGrid == true)
        {
            return Mathf.RoundToInt(position / (snapUnitSize - 0.27f)) * (snapUnitSize - 0.27f) + snapOffset;
        }
        else
        {
            return (position);
